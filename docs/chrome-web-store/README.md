...
- Future store filename after approval: `releases/linggan-boom-v2.0.94.zip`
...
```bash
npm run build
npm run release:verify -- --version 2.0.94 --zip releases/linggan-boom-v2.0.94.zip
node --test
```
...
3. After operator approval, create, verify, and upload `releases/linggan-boom-v2.0.94.zip`.
